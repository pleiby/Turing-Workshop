...
@model function dice_throw(y) # Turing models are functions of observed data
    # Priors for parameters
    p ~ Dirichlet(6, 1) # 2nd param = 1 indicates uniform over 6 probs that total to 1.0

    # Likelihood statement (prob(y|p))
    for i in eachindex(y)
        # Categorical is a type alias describingspecial case of DiscreteNonParametric dist
        y[i] ~ Categorical(p)
    end
end
```
...
model = dice_throw(my_data) # model to be fit to (informed by) these data

# NUTS with 1k iter,
# 1k warmup, and
# target acceptance rate of 80% of sample for tuning args
...
wells = CSV.read("2023-Intro-Turing/data/wells.csv", DataFrame)
describe(wells)
```
...
@model function logreg_naive(X, y; predictors=size(X, 2))
    # priors
    α ~ Normal(0, 2.5)
    β ~ filldist(TDist(3), predictors) # vector of identical distributions

    # likelihood
    for i in eachindex(y) # specify y[i] likelihood for each observation i
        y[i] ~ BernoulliLogit(α + X[i, :] ⋅ β) # logistic model regression
    end
end

model_naive = logreg_naive(X, y)

@btime sample(model_naive, NUTS(1_000, 0.8), 1_000) # 1000 samples after 1000 warmup
```
...
Only thing different is use a LazyArray over obs, for performance,
rather than separate Likelihood distribution for each observation
...
Try 4 Chains With Threading (Performance version of model using LazyArray)
...
Find: Can get ~2X speedup with LazyArray, with essentially the same est results

For benchmarking and estimating time for Bayesian execution with Turing,
see tools available at
https://github.com/TuringLang/TuringBenchmarking.jl

See SciML for excellent Julia package, e.g. for automated discovery of unknown models
with hybrid neural and Bayesian models
https://docs.sciml.ai/Overview/stable/showcase/missing_physics/

For Turing documentation, introduction and tutorials, see turinglang.org, e.g.
https://turinglang.org/v0.24/docs/using-turing/get-started
